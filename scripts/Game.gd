...
func _check_merges() -> void:
        ...
        while merged_any:
                ...
                if merged_any:
                        chain_combo += 1
                        for b1 in to_merge:
                                var b2: Node = to_merge[b1]
                                # B1 absorbs B2 and upgrades tier
                                var new_tier: int = b1.merge_into()
                                var base_score: int = b1.get_score_value() + b2.get_score_value()
...
func _apply_explosion_force(center: Vector2, radius: float, force: float) -> void:
        for block in blocks:
                if block == null or block.is_exploding:
                        continue
                var dist: float = block.global_position.distance_to(center)
                if dist < radius and dist > 0:
                        var direction: Vector2 = (block.global_position - center).normalized()
...
func _game_over() -> void:
        ...
        game_over_ui.show_game_over(score, level_data, moves_left == 0)
